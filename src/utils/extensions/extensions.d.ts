...
declare global {
    ...
    interface Promise<T> {
        ...
        thenUpdateIndex: T extends (infer U)[] ? (indexKey: keyof Extract<keyof U, number>, start: number | null | undefined) => Promise<U[]> : never;
        thenMap: T extends (infer U)[] ? <V>(callback: (value: U, index: number, array: U[]) => V) => Promise<V[]> : never;
        thenMapNotNull: T extends (infer U)[] ? <V>(callback: (value: U, index: number, array: U[]) => V | null | undefined) => Promise<V[]> : never;
    }
}
